                    ...
                    section.card id="dispatchCard" data-trait="sys.list" data-handler="refreshStats" data-interval="30000" {
                        ...
                        div.tier {
                            ...
                            div.tier-controls id="relayControls" {
                                p.note.compact-note {
                                    "Run " code { "RELAY_URL=https://relay.slob.games traits serve" } " on your Mac, enter the pairing code."
                                }
                                div.form-row.compact {
...
function copyCmd(mode) {
  var cmd = '';
  if (mode === 'run') {
    cmd = 'curl -fsSL https://slob.games/local/traits.sh | bash';
  } else {
    cmd = 'curl -fsSL https://slob.games/local/install.sh | bash';
  }
