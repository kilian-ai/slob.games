  ...
  try {
    ...
    if (result.ok === false) {
      ...
      setWorkspaceMeta(isWasm
        ? 'Chat Logs requires a local helper. Run: curl -fsSL https://www.slob.games/local/helper | bash'
        : `Error: ${result.error}`);
      return;
    }
    ...
    setWorkspaceMeta('Cannot reach local helper — start it with: curl -fsSL https://www.slob.games/local/helper | bash');
  }
}
