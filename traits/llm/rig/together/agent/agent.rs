...
pub fn rig_together_agent(args: &[Value]) -> Value {
    ...
    let model = args.get(1)
        .and_then(|v| v.as_str())
        .filter(|s| !s.is_empty())
        .unwrap_or("meta-llama/Meta-Llama-3.1-70B-Instruct-Turbo");
    ...
    let client: together::Client = together::Client::new("shim").unwrap();
    ...
    if !ok {
        let error = common::extract_error(&result, "Together API call failed");
        return json!({ "ok": false, "error": error, "provider": "rig.together.agent" });
    }
